...
`실행프로그램.html`에서 모델 5종의 형상과 Cell 1~50의 모델·배치·회전을 입력합니다. 최신 배포본은 `../20260915_145226/`의 단일 HTML 두 파일과 중앙 카메라 홀 예제 레시피입니다. PPID 확장 정의서는 `../20260914_191026/PPID.xlsm`입니다.
...
최종 보고서의 `그림으로 배우는 접촉 계산`에서 A 기준 좌표, DOE 외곽 사각형, Hole의 X/Y 겹침, 직선 Edge 여유, Round의 원호 거리 및 Gate 기록을 9개 도면과 수치 예제로 설명합니다. 중앙 Hole 1개·R 2.5 mm 예제의 JSON을 같은 폴더에서 불러올 수 있습니다. 엔진과 시뮬레이션 HTML은 그대로 유지했습니다.

해설용 경계 시험 좌표에는 0.9 mm 격자에 없는 좌표도 포함됩니다. 실제 생성 좌표 목록과 구분하며 `tests/contact-guide-examples.test.js`에서 같은 엔진으로 검증합니다.

보고서 재생성: `python docs/build-contact-guide.py YYYYMMDD_HHMMSS`
