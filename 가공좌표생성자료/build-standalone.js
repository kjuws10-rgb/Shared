...
const destination=path.resolve(process.argv[2]||path.join(root,'dist','가공좌표생성_Masking_실행프로그램.html'));
let html=fs.readFileSync(path.join(root,'실행프로그램.html'),'utf8');
html=html.replace('<link rel="stylesheet" href="./styles.css" />','<style>\n'+fs.readFileSync(path.join(root,'styles.css'),'utf8')+'\n</style>');
for(const file of ['coordinate-engine.js','masking-engine.js','masking-panel.js','app.js']) {
...
html=html.replace('저장소 기준 CSV 자동 대조','내장 기준 CSV 자동 대조').replaceAll('./가공좌표생성_최종분석보고서.html','./가공좌표생성_Masking_최종분석보고서.html');
...
fs.copyFileSync(path.join(root,'가공좌표생성_최종분석보고서.html'),path.join(path.dirname(destination),'가공좌표생성_Masking_최종분석보고서.html'));
